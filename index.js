...
const DISCORD_WARNS_FILE = path.join(DATA_DIR, 'discordwarns.json');
...
initFile(DISCORD_WARNS_FILE, {});
initFile(MODLOGS_FILE,       []);
...
function pushModLog(entry) {
  const logs = readJSON(MODLOGS_FILE);
  logs.unshift({ ...entry, timestamp: new Date().toISOString() });
  if (logs.length > 2000) logs.length = 2000;
  writeJSON(MODLOGS_FILE, logs);
}
...
function baseEmbed() {
  return new EmbedBuilder()
    ...
    .setTimestamp()
    .setFooter({ text: 'SimplyFresh Moderation', iconURL: FOOTER_ICON });
}
...
const STAFF_ROLE_ID = '1473400111685177394';
...
const commands = [
  ...
  new SlashCommandBuilder()
    .setName('dban')
    .setDescription('Ban a Discord member from the server.')
  ...
  new SlashCommandBuilder()
    .setName('dunban')
    .setDescription('Unban a Discord user from the server.')
    .addStringOption(o => o.setName('userid').setDescription('User ID to unban').setRequired(true))
    .addStringOption(o => o.setName('reason').setDescription('Reason for the unban').setRequired(false)),

  new SlashCommandBuilder()
    .setName('dkick')
    .setDescription('Kick a Discord member from the server.')
    .addUserOption(o => o.setName('user').setDescription('User to kick').setRequired(true))
    .addStringOption(o => o.setName('reason').setDescription('Reason for the kick').setRequired(false)),

  new SlashCommandBuilder()
    .setName('timeout')
    .setDescription('Timeout (mute) a Discord member.')
    .addUserOption(o => o.setName('user').setDescription('User to timeout').setRequired(true))
    .addIntegerOption(o => o.setName('duration').setDescription('Duration of the timeout').setRequired(true))
    .addStringOption(o => o.setName('unit').setDescription('Time unit').setRequired(true)
      .addChoices(
  ...
  new SlashCommandBuilder()
    .setName('untimeout')
    .setDescription('Remove timeout from a Discord member.')
    .addUserOption(o => o.setName('user').setDescription('User to untimeout').setRequired(true))
    .addStringOption(o => o.setName('reason').setDescription('Reason').setRequired(false)),

  new SlashCommandBuilder()
    .setName('dwarn')
    .setDescription('Warn a Discord member and log it.')
    .addUserOption(o => o.setName('user').setDescription('User to warn').setRequired(true))
    .addStringOption(o => o.setName('reason').setDescription('Reason for the warning').setRequired(true)),

  new SlashCommandBuilder()
    .setName('dwarnings')
    .setDescription('View Discord warnings for a member.')
    .addUserOption(o => o.setName('user').setDescription('Discord user').setRequired(true)),

  new SlashCommandBuilder()
    .setName('dclearwarnings')
    .setDescription('Clear all Discord warnings for a member.')
    .addUserOption(o => o.setName('user').setDescription('Discord user').setRequired(true)),
  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption(o => o.setName('seconds').setDescription('Slowmode delay in seconds (0 to disable, max 21600)').setRequired(true)),
  ...
  new SlashCommandBuilder()
    .setName('banlist')
    .setDescription('View the list of banned users in the server.')
    .addIntegerOption(o => o.setName('page').setDescription('Page number').setRequired(false)),

  new SlashCommandBuilder()
    .setName('modlogs')
    .setDescription('View Discord moderation logs for a user.')
    .addUserOption(o => o.setName('user').setDescription('Discord user').setRequired(true))
    .addIntegerOption(o => o.setName('page').setDescription('Page number').setRequired(false)),
];
...
// ─── SAFE REPLY HELPER ────────────────────────────────────────────────────────
// Wraps deferReply so if the interaction has already expired we bail gracefully.
...
async function safeDefer(interaction, options = {}) {
  try {
    ...
    console.error(`[safeDefer] Interaction ${interaction.id} expired before defer:`, err.code);
    return false;
  }
}
...
client.on('interactionCreate', async interaction => {
  ...
  if (interaction.isButton()) {
    ...
    if (interaction.user.id !== pending.requesterId) {
      return interaction.reply({ embeds: [errorEmbed('Only the person who ran this command can confirm it.')], ephemeral: true });
    }
    ...
    if (action === 'confirm') {
      ...
      try {
        ...
        return interaction.editReply({ embeds: [errorEmbed(`Failed: ${err.message}`)], components: [] });
      }
    }
    ...
    if (action === 'massconfirm') {
      ...
      for (const u of users) {
        try {
          await axios.patch(
            `https://groups.roblox.com/v1/groups/${GROUP_ID}/users/${u.robloxUser.id}`,
            { roleId: newRole.id },
            { headers: { Cookie: `.ROBLOSECURITY=${ROBLOX_COOKIE}`, 'X-CSRF-TOKEN': csrf } }
          );
    ...
    if (action === 'kickconfirm') {
      ...
      try {
        ...
        return interaction.editReply({ embeds: [errorEmbed(`Failed to kick: ${err.message}`)], components: [] });
      }
    }
    ...
    if (action === 'shoutconfirm') {
      ...
      try {
        ...
        return interaction.editReply({ embeds: [errorEmbed(`Failed to post shout: ${err.message}`)], components: [] });
      }
    }
  ...
  if (interaction.isModalSubmit()) {
    ...
    if (action === 'sendmodal') {
      const pending = pendingChanges.get(changeId);
      if (!pending) {
        return interaction.reply({ embeds: [errorEmbed('This session expired. Please run /send again.')], ephemeral: true });
      }
      if (interaction.user.id !== pending.requesterId) {
        return interaction.reply({ embeds: [errorEmbed('Only the person who ran this command can submit.')], ephemeral: true });
      }
      ...
      try {
        ...
        return interaction.reply({
          embeds: [baseEmbed().setDescription('✓ Embed sent successfully').addFields(
  ...
  // ════════════════════════════════════════════════════════════════════════════
  // /send — must come first since it calls showModal() as the initial response
  // ════════════════════════════════════════════════════════════════════════════
  if (commandName === 'send') {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      if (label && url) {
        try { new URL(url); buttons.push({ label, url }); }
        catch { return interaction.reply({ embeds: [errorEmbed(`Button ${i} has an invalid URL.`)], ephemeral: true }); }
      } else if (label && !url) {
    ...
    pendingChanges.set(changeId, { requesterId: interaction.user.id, channelId: channel.id, title, imageUrl, colorInput, buttons });
    setTimeout(() => pendingChanges.delete(changeId), 300_000);
    ...
    return interaction.showModal(modal); // showModal IS the response — no defer before this
  }
  ...
  if (commandName === 'changerank') {
    ...
    try {
      ...
      const bl = readJSON(BLACKLIST_FILE);
      if (bl.find(e => e.robloxId === robloxUser.id)) {
        return interaction.editReply({ embeds: [errorEmbed(`${robloxUser.name} is blacklisted from being ranked.`)] });
      }
      const roles = await getGroupRoles();
      const newRole = roles.find(r => r.name.toLowerCase() === rankName.toLowerCase());
      if (!newRole) {
        return interaction.editReply({ embeds: [errorEmbed(`Rank not found. Available: ${roles.map(r => r.name).join(', ')}`)] });
      }
  ...
  if (commandName === 'massrank') {
    ...
    if (usernameList.length > 20) {
      return interaction.reply({ embeds: [errorEmbed('Max 20 users per mass rank.')], ephemeral: true });
    }
    const ok = await safeDefer(interaction);
    if (!ok) return;
    try {
  ...
  if (commandName === 'recentranks') {
    ...
    try {
      ...
      if (!auditEntries.length) {
        return interaction.editReply({ embeds: [baseEmbed().setDescription('No recent rank changes in Roblox audit log.')] });
      }
  ...
  if (commandName === 'schedulerank') {
    ...
    if (minutes < 1 || minutes > 10080) {
      return interaction.reply({ embeds: [errorEmbed('Minutes must be between 1 and 10080 (7 days).')], ephemeral: true });
    }
    const ok = await safeDefer(interaction);
    if (!ok) return;
    try {
  ...
  if (commandName === 'schedules') {
    ...
    if (!schedules.length) {
      return interaction.reply({ embeds: [baseEmbed().setDescription('No pending scheduled rank changes.')] });
    }
  ...
  if (commandName === 'cancelschedule') {
    ...
    if (idx === -1) {
      return interaction.reply({ embeds: [errorEmbed(`Schedule \`${id}\` not found or already executed.`)], ephemeral: true });
    }
  ...
  if (commandName === 'ranklog') {
    ...
    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**Rank Log — Page ${pageNum}/${totalPages}**\n\n${lines}`)
      .setFooter({ text: `SimplyFresh Moderation • ${logs.length} total entries`, iconURL: FOOTER_ICON })] });
  }
  ...
  if (commandName === 'stafflog') {
    ...
    if (!filtered.length) {
      return interaction.reply({ embeds: [baseEmbed().setDescription(`No rank changes found for <@${staffUser.id}>.`)] });
    }
    ...
    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**Staff Log for <@${staffUser.id}> — Page ${pageNum}/${totalPages}**\n\n${lines}`)
      .setFooter({ text: `SimplyFresh Moderation • ${filtered.length} total actions`, iconURL: FOOTER_ICON })] });
  }
  ...
  if (commandName === 'userlog') {
    ...
    if (!filtered.length) {
      return interaction.reply({ embeds: [baseEmbed().setDescription(`No rank changes found for **${username}**.`)] });
    }
    const totalPages = Math.ceil(filtered.length / PER_PAGE);
    const pageNum    = Math.min(page, totalPages);
    const slice      = filtered.slice((pageNum - 1) * PER_PAGE, pageNum * PER_PAGE);
    ...
    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**Rank history for \`${username}\` — Page ${pageNum}/${totalPages}**\n\n${lines}`)
      .setFooter({ text: `SimplyFresh Moderation • ${filtered.length} total changes`, iconURL: FOOTER_ICON })] });
  }
  ...
  if (commandName === 'verify') {
    ...
    try {
      ...
      if (existing && existing[0] !== interaction.user.id) {
        return interaction.editReply({ embeds: [errorEmbed('That Roblox account is already linked to another Discord user.')] });
      }
  ...
  if (commandName === 'unverify') {
    ...
    if (!verified[interaction.user.id]) {
      return interaction.reply({ embeds: [errorEmbed('You are not verified.')], ephemeral: true });
    }
  ...
  if (commandName === 'whois') {
    ...
    try {
      ...
      const [robloxUser, role, avatar] = await Promise.all([
        getRobloxUserById(entry.robloxId),
        getUserGroupRole(entry.robloxId),
  ...
  if (commandName === 'checkinventory') {
    ...
    try {
      ...
      return interaction.editReply({ embeds: [baseEmbed()
        .setDescription(owns ? `✓ User owns this ${type}` : `✗ User does not own this ${type}`)
        .addFields(
          { name: 'User',       value: robloxUser.name },
  ...
  if (commandName === 'rankingaccount') {
    ...
    try {
      ...
      const [avatar, role] = await Promise.all([getRobloxAvatar(account.id), getUserGroupRole(account.id)]);
  ...
  if (commandName === 'blacklist') {
    ...
    try {
      ...
      const bl         = readJSON(BLACKLIST_FILE);
      if (bl.find(e => e.robloxId === robloxUser.id)) {
        return interaction.editReply({ embeds: [errorEmbed(`${robloxUser.name} is already blacklisted.`)] });
      }
  ...
  if (commandName === 'unblacklist') {
    ...
    try {
      ...
      if (!bl.find(e => e.robloxId === robloxUser.id)) {
        return interaction.editReply({ embeds: [errorEmbed(`${robloxUser.name} is not blacklisted.`)] });
      }
  ...
  if (commandName === 'watchlist') {
    ...
    if (!username) {
      return interaction.reply({ embeds: [errorEmbed('Username is required for add/remove.')], ephemeral: true });
    }
    const ok = await safeDefer(interaction);
    if (!ok) return;
    try {
      ...
      if (action === 'add') {
        if (wl.find(e => e.robloxId === robloxUser.id)) {
          return interaction.editReply({ embeds: [errorEmbed(`${robloxUser.name} is already on the watchlist.`)] });
        }
      ...
      if (action === 'remove') {
        if (!wl.find(e => e.robloxId === robloxUser.id)) {
          return interaction.editReply({ embeds: [errorEmbed(`${robloxUser.name} is not on the watchlist.`)] });
        }
  ...
  if (commandName === 'resolveappeal') {
    ...
    if (!['approve', 'deny'].includes(decision)) {
      return interaction.reply({ embeds: [errorEmbed('Decision must be "approve" or "deny".')], ephemeral: true });
    }
    const appeals = readJSON(APPEALS_FILE);
    const appeal  = appeals.find(a => a.id === id);
    if (!appeal) return interaction.reply({ embeds: [errorEmbed(`Appeal \`${id}\` not found.`)], ephemeral: true });
    if (appeal.status !== 'pending') {
      return interaction.reply({ embeds: [errorEmbed(`Appeal \`${id}\` is already resolved.`)], ephemeral: true });
    }
    ...
    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`✓ Appeal ${decision === 'approve' ? 'approved' : 'denied'}`)
      .addFields(
        { name: 'Appeal ID',  value: id },
  ...
  if (commandName === 'shout') {
    ...
    if (message.length > 255) {
      return interaction.reply({ embeds: [errorEmbed('Shout must be 255 characters or less.')], ephemeral: true });
    }
    const ok = await safeDefer(interaction);
    if (!ok) return;
    const changeId = `${interaction.id}-${Date.now()}`;
  ...
  if (commandName === 'own') {
    ...
    try {
      ...
      return interaction.editReply({ embeds: [baseEmbed()
        .setDescription(owns ? '✓ Game pass owned' : '✗ Game pass not owned')
        .addFields(
          { name: 'User',         value: robloxUser.name },
  ...
  if (commandName === 'help') {
    const sections = {
      ...
      'Roblox Moderation':  '`/warn` `/warnings` `/clearwarnings` `/addnote` `/notes` `/clearnotes` `/kick`',
      ...
      'Embed':              '`/send`',
      'Discord Moderation': '`/dban` `/dunban` `/dkick` `/timeout` `/untimeout` `/dwarn` `/dwarnings` `/dclearwarnings` `/purge` `/slowmode` `/lock` `/unlock` `/banlist` `/modlogs`',
    };
    const fields = Object.entries(sections).map(([name, value]) => ({ name, value }));
    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**SimplyFresh Moderation — All Commands**\nMade by **Zaid** • ${commands.length} total commands`)
      .addFields(...fields)], ephemeral: true });
  }
  ...
  if (commandName === 'dban') {
    ...
    if (target.id === client.user.id) {
      return interaction.reply({ embeds: [errorEmbed('You cannot ban the bot.')], ephemeral: true });
    }
    ...
    try {
      ...
      if (member) {
        if (!member.bannable) return interaction.editReply({ embeds: [errorEmbed(`I cannot ban **${target.tag}** — they may have a higher role than me.`)] });
      }

      await interaction.guild.members.ban(target.id, { reason, deleteMessageDays: deleteDays });

      pushModLog({ action: 'BAN', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: target.id, targetTag: target.tag, reason });
      pushAudit('DBAN', interaction.user.tag, interaction.user.id, `${target.tag} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('🔨 Member Banned').addFields(
        { name: 'User',         value: `${target.tag} (${target.id})` },
        { name: 'Reason',       value: reason },
        { name: 'Msgs Deleted', value: `${deleteDays} day(s)` },
        { name: 'Banned By',    value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription('🔨 Member banned').addFields(
  ...
  if (commandName === 'dunban') {
    ...
    if (!/^\d{17,20}$/.test(userId)) {
      return interaction.reply({ embeds: [errorEmbed('Invalid user ID. Must be a valid Discord user ID (17–20 digits).')], ephemeral: true });
    }
    ...
    try {
      ...
      if (!ban) return interaction.editReply({ embeds: [errorEmbed(`User \`${userId}\` is not banned.`)] });

      await interaction.guild.members.unban(userId, reason);

      pushModLog({ action: 'UNBAN', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: userId, targetTag: ban.user.tag, reason });
      pushAudit('DUNBAN', interaction.user.tag, interaction.user.id, `${ban.user.tag} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('✓ Member Unbanned').addFields(
        { name: 'User',       value: `${ban.user.tag} (${userId})` },
        { name: 'Reason',     value: reason },
        { name: 'Unbanned By',value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription('✓ Member unbanned').addFields(
  ...
  if (commandName === 'dkick') {
    ...
    if (target.id === client.user.id) {
      return interaction.reply({ embeds: [errorEmbed('You cannot kick the bot.')], ephemeral: true });
    }
    ...
    try {
      const member = await interaction.guild.members.fetch(target.id).catch(() => null);
      if (!member) return interaction.editReply({ embeds: [errorEmbed(`${target.tag} is not in this server.`)] });
      if (!member.kickable) return interaction.editReply({ embeds: [errorEmbed(`I cannot kick **${target.tag}** — they may have a higher role than me.`)] });

      await member.kick(reason);

      pushModLog({ action: 'KICK', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: target.id, targetTag: target.tag, reason });
      pushAudit('DKICK', interaction.user.tag, interaction.user.id, `${target.tag} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('👢 Member Kicked').addFields(
        { name: 'User',     value: `${target.tag} (${target.id})` },
        { name: 'Reason',   value: reason },
        { name: 'Kicked By',value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription('👢 Member kicked').addFields(
        { name: 'User',     value: target.tag, inline: true },
        { name: 'User ID',  value: target.id,  inline: true },
        { name: 'Reason',   value: reason },
        { name: 'Kicked By',value: `<@${interaction.user.id}>` }
      ...
      return interaction.editReply({ embeds: [errorEmbed(`Failed to kick: ${err.message}`)] });
    }
  }
  ...
  if (commandName === 'timeout') {
    ...
    if (target.id === interaction.user.id) {
      return interaction.reply({ embeds: [errorEmbed('You cannot timeout yourself.')], ephemeral: true });
    }
    if (target.id === client.user.id) {
      return interaction.reply({ embeds: [errorEmbed('You cannot timeout the bot.')], ephemeral: true });
    }

    const unitMultipliers = { seconds: 1000, minutes: 60_000, hours: 3_600_000, days: 86_400_000 };
    const ms = duration * unitMultipliers[unit];
    ...
    try {
      const member = await interaction.guild.members.fetch(target.id).catch(() => null);
      if (!member) return interaction.editReply({ embeds: [errorEmbed(`${target.tag} is not in this server.`)] });
      if (!member.moderatable) return interaction.editReply({ embeds: [errorEmbed(`I cannot timeout **${target.tag}** — they may have a higher role than me.`)] });

      await member.timeout(ms, reason);

      const displayDuration = `${duration} ${unit}`;
      const expiresAt = new Date(Date.now() + ms).toLocaleString('en-GB');

      pushModLog({ action: 'TIMEOUT', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: target.id, targetTag: target.tag, reason, duration: displayDuration });
      pushAudit('TIMEOUT', interaction.user.tag, interaction.user.id, `${target.tag} — ${displayDuration} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('🔇 Member Timed Out').addFields(
        { name: 'User',       value: `${target.tag} (${target.id})` },
        { name: 'Duration',   value: displayDuration },
        { name: 'Expires',    value: expiresAt },
        { name: 'Reason',     value: reason },
        { name: 'Timed Out By', value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription('🔇 Member timed out').addFields(
        { name: 'User',         value: target.tag,    inline: true },
        { name: 'User ID',      value: target.id,     inline: true },
      ...
      return interaction.editReply({ embeds: [errorEmbed(`Failed to timeout: ${err.message}`)] });
    }
  }
  ...
  if (commandName === 'untimeout') {
    ...
    try {
      ...
      if (!member.communicationDisabledUntil) {
        return interaction.editReply({ embeds: [errorEmbed(`${target.tag} is not currently timed out.`)] });
      }

      await member.timeout(null, reason);

      pushModLog({ action: 'UNTIMEOUT', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: target.id, targetTag: target.tag, reason });
      pushAudit('UNTIMEOUT', interaction.user.tag, interaction.user.id, `${target.tag} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('🔊 Timeout Removed').addFields(
        { name: 'User',          value: `${target.tag} (${target.id})` },
        { name: 'Reason',        value: reason },
        { name: 'Removed By',    value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription('🔊 Timeout removed').addFields(
        { name: 'User',       value: target.tag, inline: true },
        { name: 'User ID',    value: target.id,  inline: true },
        { name: 'Reason',     value: reason },
        { name: 'Removed By', value: `<@${interaction.user.id}>` }
      ...
      return interaction.editReply({ embeds: [errorEmbed(`Failed to remove timeout: ${err.message}`)] });
    }
  }

  // ── /dwarn ────────────────────────────────────────────────────────────────
  if (commandName === 'dwarn') {
    if (!hasPermission(interaction.member)) {
      return interaction.reply({ embeds: [errorEmbed('No permission.')], ephemeral: true });
    }
    const target = interaction.options.getUser('user');
    ...
    if (!ok) return;
    try {
      const dwarns = readJSON(DISCORD_WARNS_FILE);
      if (!dwarns[target.id]) dwarns[target.id] = [];
      dwarns[target.id].push({
        reason,
        staffId:  interaction.user.id,
        staffTag: interaction.user.tag,
        timestamp: new Date().toISOString()
      });
      writeJSON(DISCORD_WARNS_FILE, dwarns);

      pushModLog({ action: 'WARN', staffId: interaction.user.id, staffTag: interaction.user.tag, targetId: target.id, targetTag: target.tag, reason });
      pushAudit('DWARN', interaction.user.tag, interaction.user.id, `${target.tag} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('⚠️ Member Warned').addFields(
        { name: 'User',           value: `${target.tag} (${target.id})` },
        { name: 'Reason',         value: reason },
        { name: 'Total Warnings', value: String(dwarns[target.id].length) },
      ...
      return interaction.editReply({ embeds: [baseEmbed().setDescription('⚠️ Member warned').addFields(
        { name: 'User',           value: target.tag,                        inline: true },
        { name: 'User ID',        value: target.id,                         inline: true },
        { name: 'Reason',         value: reason },
        { name: 'Total Warnings', value: String(dwarns[target.id].length),  inline: true },
        { name: 'Warned By',      value: `<@${interaction.user.id}>`,       inline: true }
      )] });
    } catch (err) {
      return interaction.editReply({ embeds: [errorEmbed(`Failed to warn: ${err.message}`)] });
    }
  }

  // ── /dwarnings ────────────────────────────────────────────────────────────
  if (commandName === 'dwarnings') {
    if (!hasPermission(interaction.member)) {
      return interaction.reply({ embeds: [errorEmbed('No permission.')], ephemeral: true });
    }
    const target  = interaction.options.getUser('user');
    const dwarns  = readJSON(DISCORD_WARNS_FILE)[target.id] || [];

    if (!dwarns.length) {
      return interaction.reply({ embeds: [baseEmbed().setDescription(`No Discord warnings for **${target.tag}**.`)] });
    }

    const lines = dwarns.map((w, i) => {
      const date = new Date(w.timestamp).toLocaleDateString('en-GB');
      return `**${i + 1}.** ${w.reason}\n> by ${w.staffTag} • ${date}`;
    }).join('\n\n');

    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**Discord Warnings for ${target.tag} (${dwarns.length} total)**\n\n${lines}`)
    ] });
  }

  // ── /dclearwarnings ───────────────────────────────────────────────────────
  if (commandName === 'dclearwarnings') {
    if (!hasPermission(interaction.member)) {
      return interaction.reply({ embeds: [errorEmbed('No permission.')], ephemeral: true });
    }
    const target = interaction.options.getUser('user');
    const dwarns = readJSON(DISCORD_WARNS_FILE);
    const prev   = (dwarns[target.id] || []).length;
    dwarns[target.id] = [];
    writeJSON(DISCORD_WARNS_FILE, dwarns);
    pushAudit('DCLEARWARNINGS', interaction.user.tag, interaction.user.id, `${target.tag} — cleared ${prev} warning(s)`);

    return interaction.reply({ embeds: [baseEmbed().setDescription(`✓ Cleared **${prev}** Discord warning(s) for **${target.tag}**.`)] });
  }
  ...
  if (commandName === 'purge') {
    ...
    try {
      ...
      const deletable   = (filterUser ? [...messages.values()] : [...messages.values()]).filter(m => m.createdTimestamp > twoWeeksAgo);

      if (!deletable.length) {
        return interaction.editReply({ embeds: [errorEmbed('No messages found that can be deleted (must be under 14 days old).')] });
      }

      const deleted = await interaction.channel.bulkDelete(deletable, true);

      pushAudit('PURGE', interaction.user.tag, interaction.user.id, `Deleted ${deleted.size} messages in #${interaction.channel.name}${filterUser ? ` from ${filterUser.tag}` : ''}`);

      await sendLogToChannel(baseEmbed().setDescription('🗑️ Messages Purged').addFields(
        { name: 'Channel',   value: `<#${interaction.channel.id}>` },
        { name: 'Deleted',   value: String(deleted.size) },
        { name: 'Filter',    value: filterUser ? filterUser.tag : 'All messages' },
        { name: 'Purged By', value: `<@${interaction.user.id}>` }
      ));

      return interaction.editReply({ embeds: [baseEmbed().setDescription(`🗑️ Successfully deleted **${deleted.size}** message(s)${filterUser ? ` from ${filterUser.tag}` : ''}.`)] });
    } catch (err) {
      return interaction.editReply({ embeds: [errorEmbed(`Failed to purge: ${err.message}`)] });
    }
  }
  ...
  if (commandName === 'slowmode') {
    ...
    if (seconds < 0 || seconds > 21600) {
      return interaction.reply({ embeds: [errorEmbed('Slowmode must be between 0 (off) and 21600 seconds (6 hours).')], ephemeral: true });
    }

    try {
      await interaction.channel.setRateLimitPerUser(seconds);
      pushAudit('SLOWMODE', interaction.user.tag, interaction.user.id, `#${interaction.channel.name} → ${seconds}s`);

      const display = seconds === 0 ? 'Disabled' : `${seconds} second(s)`;
      return interaction.reply({ embeds: [baseEmbed().setDescription(`🐢 Slowmode updated`).addFields(
        { name: 'Channel',    value: `<#${interaction.channel.id}>`, inline: true },
        { name: 'Slowmode',   value: display,                         inline: true },
  ...
  if (commandName === 'lock') {
    ...
    try {
      ...
      pushAudit('LOCK', interaction.user.tag, interaction.user.id, `#${interaction.channel.name} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('🔒 Channel Locked').addFields(
        { name: 'Channel',   value: `<#${interaction.channel.id}>` },
        { name: 'Reason',    value: reason },
        { name: 'Locked By', value: `<@${interaction.user.id}>` }
      ));

      return interaction.reply({ embeds: [baseEmbed().setDescription('🔒 Channel locked — members can no longer send messages.').addFields(
        { name: 'Channel',   value: `<#${interaction.channel.id}>`, inline: true },
        { name: 'Reason',    value: reason },
        { name: 'Locked By', value: `<@${interaction.user.id}>` }
      )] });
    } catch (err) {
      return interaction.reply({ embeds: [errorEmbed(`Failed to lock channel: ${err.message}`)], ephemeral: true });
    }
  }
  ...
  if (commandName === 'unlock') {
    ...
    try {
      ...
      pushAudit('UNLOCK', interaction.user.tag, interaction.user.id, `#${interaction.channel.name} — ${reason}`);

      await sendLogToChannel(baseEmbed().setDescription('🔓 Channel Unlocked').addFields(
        { name: 'Channel',     value: `<#${interaction.channel.id}>` },
        { name: 'Reason',      value: reason },
        { name: 'Unlocked By', value: `<@${interaction.user.id}>` }
      ));

      return interaction.reply({ embeds: [baseEmbed().setDescription('🔓 Channel unlocked — members can send messages again.').addFields(
        { name: 'Channel',     value: `<#${interaction.channel.id}>`, inline: true },
        { name: 'Reason',      value: reason },
        { name: 'Unlocked By', value: `<@${interaction.user.id}>` }
      )] });
    } catch (err) {
      return interaction.reply({ embeds: [errorEmbed(`Failed to unlock channel: ${err.message}`)], ephemeral: true });
    }
  }
  ...
  if (commandName === 'banlist') {
    ...
    try {
      ...
      if (!bans.size) return interaction.editReply({ embeds: [baseEmbed().setDescription('No banned users in this server.')] });

      const banArray   = [...bans.values()];
      const totalPages = Math.ceil(banArray.length / PER_PAGE);
      const pageNum    = Math.min(page, totalPages);
      const slice      = banArray.slice((pageNum - 1) * PER_PAGE, pageNum * PER_PAGE);

      const lines = slice.map((ban, i) =>
        `**${(pageNum - 1) * PER_PAGE + i + 1}.** ${ban.user.tag} (\`${ban.user.id}\`)\n> ${ban.reason || 'No reason provided.'}`
      ).join('\n\n');

      return interaction.editReply({ embeds: [baseEmbed()
        .setDescription(`**Server Ban List — Page ${pageNum}/${totalPages}**\n\n${lines}`)
        .setFooter({ text: `SimplyFresh Moderation • ${bans.size} total bans`, iconURL: FOOTER_ICON })
      ] });
    } catch (err) {
      return interaction.editReply({ embeds: [errorEmbed(`Failed to fetch ban list: ${err.message}`)] });
    }
  }
  ...
  if (commandName === 'modlogs') {
    ...
    const all      = readJSON(MODLOGS_FILE).filter(l => l.targetId === target.id);
    if (!all.length) {
      return interaction.reply({ embeds: [baseEmbed().setDescription(`No moderation logs found for **${target.tag}**.`)] });
    }

    const totalPages = Math.ceil(all.length / PER_PAGE);
    const pageNum    = Math.min(page, totalPages);
    const slice      = all.slice((pageNum - 1) * PER_PAGE, pageNum * PER_PAGE);

    const actionEmoji = { BAN: '🔨', UNBAN: '✓', KICK: '👢', TIMEOUT: '🔇', UNTIMEOUT: '🔊', WARN: '⚠️' };

    const lines = slice.map((log, i) => {
      const date  = new Date(log.timestamp).toLocaleDateString('en-GB');
      const emoji = actionEmoji[log.action] || '•';
      const extra = log.duration ? ` (${log.duration})` : '';
      return `**${(pageNum - 1) * PER_PAGE + i + 1}.** ${emoji} \`${log.action}\`${extra}\n> ${log.reason || 'No reason.'} • by ${log.staffTag} • ${date}`;
    }).join('\n\n');

    return interaction.reply({ embeds: [baseEmbed()
      .setDescription(`**Mod Logs for ${target.tag} — Page ${pageNum}/${totalPages}**\n\n${lines}`)
      .setFooter({ text: `SimplyFresh Moderation • ${all.length} total actions`, iconURL: FOOTER_ICON })
    ] });
  }
});
...
client.once('ready', async () => {
  ...
  client.user.setActivity('SimplyFresh Moderation', { type: ActivityType.Watching });
  ...
  if (process.env.AUDIT_LOG_CHANNEL) {
    ...
    console.log('⚠️  AUDIT_LOG_CHANNEL not set — Discord audit log disabled.');
  }
});
